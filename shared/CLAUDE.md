...
> **最后更新**：2026-08-04（实习线归档 + ripple-site 定案 + SBS 过线；此前 2026-05-25：NAISC 终局 + §3 frame shift + §9 模式系统 + §8 consolidation）
...
| **Ripple (core + iOS + site)**<br>🔴 **紧急**(§5.0) | **⭐ 一个大项目**:`ripple-core` + `ripple-ios` + `ripple-site` **三仓 2026-07-25 经 owner 合并,不再分开排期** · **✅ 已上架 App Store(2026-07-14 过审自动发布,第三次提交成功;商店名 "Ripple Health AI",免费,iOS 18.0+,Apple ID 6786394791)** · 后端 live · LLM=OpenAI `gpt-5.4-mini` 独家(数据不进中国) · post-launch:**Supabase Pro ✅ 2026-07-28 已升级**(每日备份实证)/OpenAI 用量上限=owner 自有门规/**Vercel 防火墙暂不做**(hobby 档,WAF 要 Vercel Pro $20/月;cost-guard 应用层限流已够)/**A-loop(07-28,A1-A7 七轮)已交付:A3 后台同步工程+监控(uptime workflow 实测绿+error_log+admin/errors)+SIWA 撤销全链(env 缺省 no-op 已上线)**;**B-loop(07-28 深夜,B1-B6 六轮)已交付「留存三件套」:W1 首日体验(first-read 端点+即时基线重算+空态三形态)/W2 晨间简报(每本地日一条+7-9 窗+无数据不发)/W3 第一方埋点+漏斗(首份快照:30 天真实注册 13→连表 3→解读 0→回访 0,**连表是最大漏水口**)**;**C-loop(07-29 凌晨,C1-C5)基线补强毕:onboarding 埋点+评分引导/引用验活+漏斗周报 workflow/功能性 a11y(VoiceOver 首次可用长按)/合规三件 DRAFT/错误态 sweep**;dev 构建已装 owner 真机(含全部新码);**两把 .p8 已通电(07-29:owner 生成,agent 配 production+三重验证——owner 真机实收测试推送与当天真晨报 apns:1,SIWA 探针 invalid_grant;私钥 ~/Documents/RippleKeys/)**;**D-loop + E-loop(07-29 深夜→07-30 06:35,九段全毕并验证)= owner 亲自点出的 UI/AI 呈现问题全面重做**:D1 文案人话化(裸 metric key 不再出现在人眼前)/D2 日期守卫(未来日与空日禁长按,不再为空白日烧 LLM)/**D3 调查真 SSE —— 途中发现全 app 最大的一个 bug:`api/index.ts` 把响应体全部读完才发第一个字节,每个 SSE 端点从来没有真正流过(14 事件全在 11.12s 同时到达;修后 created 1.45s→verdict 13.9s)**,并写了显式测试保证重连绝不重跑 agent(烧钱 bug)/D4 Explain 两段式(先证据后结论,删掉叙述已完成工作的假 ticker)/E1 底部 tab 分离(日历/Notice/Me)/E2 Notice 重设计(今天-昨天-本周-更早分组 + 绝对日期 + tap=朴素事实、hold=AI)/E3 日历重设计(短按进当日页、内联面板退休、标记改颜色+形状双通道解决色盲不可读)/E4 当日页基础信息在前 AI 为可选深入(新增本地零 LLM 概览句 + 「Ripple 在这天注意到的」段,修掉"日历红点说有事、点进去一个字不提"的断裂)/E5 AI 呈现语言统一(confidence 两套实现合一并先给话再给条、弃权与免责在所有结论面一致、图表先用话介绍再画)。**gate**:iOS 116/116 单测 + 20/20 UITests,core 173/173 + 部署 + 远程冒烟,并逐轮截图实看。**build5-loop(07-31 过夜,L1-L6 六段全毕并验证)= 07-30 评审会全部可执行项落地**:L1 清账(**那个「算法 bug」实为标签错**——`deviation_pct` 是「7 天均值 vs 30 天基线」,却被摆在「那天/常态」下当成两数之差;现改为**凡两数同屏就地算**)+ 差评可撤销 + **全 app 左滑返回**(根因:隐藏导航栏会连带关掉系统侧滑手势)+ investigation 返回不重跑 / **P1 四 tab:Home=今天、日历自立门户**(Home 与当日页共用同一组件;**今天成为一等空态**——Home=今天意味着每天清晨天然是空的)/ L2 Notice 行改「指标+箭头+变化+小趋势图」、文字下沉,**百分比由后端结构化返回而非解析自由文本**;图表**关键点直接标数值**(手机无 hover 可退守)/ L3 **日历色阶做成单色相×强度**(实测绿→红亮度**非单调**、中间档最亮,灰度下语义会反;**灰度截图验收通过**)+ **P5 后端长杆先起**(anomaly 区间**派生不建表**;区间分析扩展 `/v1/explain` 的 `range` 主体**而非新开端点**——少一整套合规面;**「重连绝不重跑 agent」写成显式测试**)+ 🔖 **`build5-fallback` tag(可独立送审)** / L4 长按 notice **弹出问题区间**(`matchedGeometryEffect`+`.ultraThinMaterial`+复用既有充能环)/ L5 **框选区间→AI 分析**+本周/本月预设 —— 这是「不做 chatbot 但能主动提问」的落地 / L6 合规自查(beta 零命中、权限按钮措辞、**11 条引用 curl 全 200**、**同意门按数据流排查**)。**gate**:iOS 122/122 单测 + 24/24 UITests,core 189/189 + 部署 + 远程冒烟,逐段截图实看。⏸ owner 仅剩:**build 5 发版决定**(A/B/C/D/E/**build5 全部改动**要它才到 App Store 用户;**先决定送 main 还是送 `build5-fallback`**,见 `ripple-ios/docs/BUILD5-OWNER-CHECKLIST.md`)+ **AI 表达理念未定**(解释原因/只给建议/用户可选,卡住 Notice 那句「一句话原因」)+ 官方 email + 真机余项(A3 过夜查库/VoiceOver/评分弹窗)+ **home 页构图决定** + **框选是否要做成裸手势**(现为显式模式,原因见 checklist §4)(E1+E3 把月历以下掏空了,agent 故意没发明新模块去填,已列 UI-POLISH 第一条) | NAISC 后重写的多租户 wellness agent。后端 `ripple-core`（Hono 单 Vercel 函数 + Supabase `ubuamehrsvyrbnoxtavk`，[ripple-core.vercel.app](https://ripple-core.vercel.app)：三层 investigation agent + 53 条规则 + 结构化 explain/答案缓存 + Web Push/APNs 双通道 + softAuth 安全默认，迁移到 0022、npm 145/145）。原生 iOS `ripple-ios`（SwiftUI，min iOS 18.0，bundle `com.ripplehealth.ios`）：日历 Home + 长按 Explain Sheet 作为**唯一 AI 入口**(无 chatbot/dashboard)，HealthKit 前台回填已完成;**A3 后台增量同步(YOR-109)工程 2026-07-28 A-loop 建成**(background-delivery entitlement 签名实证 + anchor 增量读 + 观察者×8/15min 节流/设备令牌上传 + 离线队列/BGProcessingTask sync-drain;单测 54/54+UITests 13/13,**仅真机端到端验证 ⏸ owner**)。TestFlight 已跳过;Sign in with Apple 已接入并 2026-07-05 真机验证通过(该风险已消)。iOS owner = Gengyue；凭据 2026-07-02 全轮换。旧 web 前端 `ripple`(yorhagengyue/ripple, ripple-wellness.vercel.app)已**退役**(peer console 导出到 `~/ripple-peer-archive/`)。**Active web = 营销官网 [`toffemoon/ripple-site`](https://github.com/toffemoon/ripple-site)**(2026-07-21 定案:React 18+Vite+Tailwind 4 单页,App Store 下载 CTA 直链 id6786394791;track-B B1 功能型 SPA 舍弃——Linear YOR-123~131 Canceled、YOR-132/133 由官网完成标 Done、YOR-134/135 留 Backlog)。<br>synced **2026-08-02 晚**(两条线:①**夜审+跟进修完 4 个生产缺陷**——摄取单位(HAE 千焦被当千卡存,4.18×,解析器从没读过 units 字段;修+`extra.unit_in` 落档+线上验证)/**累计指标日值 = 行求和的一个数量级级错误**(一天 11 行是「当日累计快照」不是切片,`mean×n` 把 05-23 显示成 142,193 步、真实 20,430;5 处调用点归一成 `cumulative.ts dailyValue()` 取 max,生产实测 20,430)/watchdog 逐用户隔离(账号在扫描中途被删→FK 23503 杀掉整晚 cron;skip 计数**返回在 summary 里**)/两条 5s 默认超时的假红(account-delete、cron/baseline→20s,附实测数据)。**大事纠正:owner 确认「Tommy的Apple Watch」7315 行是他自己的旧表**——我误判为他人数据并加的 90 天下限**已撤**(会挡他自己 7005 行历史);真缺陷是「一分钟 7315 行没人看得见」,N18 埋点已是正解。夜审报告两条最响结论就地划掉(见 §8 同日 correction)。②**主页构图定案落地(owner 拍板 Bevel 参照:借语法、拒 0-100 分)**——「LAST 7 DAYS」四张趋势卡(`MetricLabel.status` 词汇+SparkBars+新鲜度戳"61 · Sat",**高亮过的指标不再出卡**防一信号两答案;band-dot 押后到 worn-day 基线)+问候语+日期常驻(化解 K4 与 07-30 日期之争)。选卡纯逻辑 `HomeGlance.swift` 进测试目标(7 测)。门:core 237/237+部署+demo 身份线上探针,iOS 162/162+UITests+冷启动截图实看(四卡+排除+回退行全对)。⏸ owner 新增:历史快照收敛(1850 行中间快照→222 行日总量,删除类,方案在 MORNING-REPORT「Defect 3」)+空 Home 的结构性根因(n_30d==n_7d ⇒ deviation≡0)要 worn-day 基线设计。真源 LOOP-PROGRESS W/S-round + MORNING-REPORT 附录。**S-round(同晚,owner 报「一会没问题一会 Signal」)**:根因=**两个基线算法在打架**——夜间 cron 用日总量+25%/7% 阈值,而**每次打开 app 的 first-read 用旧算法**(原始行当样本、快照各算一条、kJ 混入、一刀切 7%),开 app 出 Signal、夜里 cron 又抹掉;0031 三函数统一定义并实证 `_user`/`_active` 产出相同行。0032 历史修复:1851 条当日累计快照坍缩成每日峰值行 + owner 33 条导出器能量行 kJ→kcal(按 source 圈定、证据门控;另一 HAE 用户无证据不动)。**新坑归档:iOS 设备名里是 U+00A0**——`source='耿越的Apple Watch'` 普通空格等号/LIKE 全落空,hex 才看得见。owner 账户 active_energy 基线 2856→1120 kcal、watch→normal,幻影 Signal 源头消灭;237/237。**app 端两源同晚做完(owner「两个都做」)**:①keep-prior——拉取失败≠数据为空,anomalies 失败保留上次证据行、角标只在拿到新数据时发布,week/回退探针同理;②回退门从「今天为空」改成「今天可答」(TodayGate:有瞬时体征信号才接管页面),0→1 条读数的清晨悬崖消灭,每天只翻一次页;连带把 K-loop 挂账的「回退藏掉今天计划」修掉——读数可以是周六的、PLANS 永远是今天的,文案跟着改「1 plan today」;③LiveSmokeTests 自愈 JWT(种过期令牌实证:自动换新+写回+15/15 绿),今天第三次假红从机制上终结。iOS 167/167+UITests 3/3+冷启动截图逐项实看)· 前次 synced 2026-08-02 早(**K-loop 五轮完成:主页真正变成状态页**。K1 `SignalGlance` 六个用户可见缺陷+拆文件让该位置可测(零覆盖根因=与 View 同文件、物理上进不了 logic-test target)/K2 删两个分区标题与卡片边框、**证据行过滤为 `metric ∈ offUsual`** 替代 E5 用标题做的灭火器、上限两条/K3 九行按状态劈分+去重、**删掉一条在过滤器之下不可能说真话的空态文案**/K4 排版层级倒转(状态词全页最大且白)、状态块零色相、回退行改中性灰、主页隐藏空 PLANS/K5 一秒涟漪转场(Reduce Motion 无静态替代品)+Notice 角标(**只数主页没渲染出来的 open case**)。整页 2.2 屏→1 屏,HRV 三次→两次。门:单测 146/0/1 + UITests 25/25 + 0 restart,每轮模拟器实看;跨午夜时真实触发 H2 回退并一次性验证全部改动。⏸ owner:**主页构图(底部约 55% 空,属视觉创作未自行填)**/日期降级是否接受(与 07-30「左上角看清 31 号」有张力)/涟漪观感/build5 发版。⚠️ 交下轮:**回退把今天的计划也一起藏了**(读数属过去、计划属未来,绑在同一次回退可能是错的)/notice delta 未必是触发量(种子数据不能当生产证据)/冷启动加载态单薄。真源 `ripple-core/docs/LOOP-PROGRESS.md` K1-K5 段)· **1.1.0 (7) 发版轮(2026-08-02 深夜)**:图表点击→详情页 hover 联动(点位放大+游标线+对应行高亮滚入,纯逻辑 nearestDataSlot/slotDay 进测试)、周卡片图表语言重做(体征=线+点+日历空档,数量=零基线柱;修掉 min-max 把 Usual 画成风暴 + 缺勤日被压缩两个真缺陷)、拖选跨机型修复(LongPress maximumDistance 10→60,13 Pro 只能单选的根因)、demo 账号限流全开(owner 令;DEMO_USER_ID 生产原是空串一并修,402→200 实证)、429 谜案结案(护栏下沉到缓存未命中才计数)。**Release 归档+上传一次过(Upload succeeded),三台真机(17PM/13P/15PM)全部 1.1.0 (7)**;门=单测 171/171+UITests 27/27+后端 240/240。⏸ owner:ASC 登录后手动送审(What’s New 已给)· **L10N loop(2026-08-04 凌晨,Z1-Z7 七轮全毕)= 中文版建成**:全 UI 简中(String Catalog 454 keys,默认跟随系统,Me→语言 可选 跟随系统/English/简体中文,下次启动生效)+ **AI 回答跟随语言**(personalize() 在 guidance 咽喉点一次覆盖 explain/investigation,watchdog/晨报单独接,prose 中文而 JSON keys/enum 保持英文,生产实证中文 verdict)+ 品牌词表(静水/一道涟漪/留意/平常,PLAN-L10N.md 可否决)。终门=单测 176/176+UITests 29/29(27 EN+2 zh 冒烟)+后端 242/242 已部署;1.1.0 (8) 三台真机全装。真源 ripple-ios/docs/PLAN-L10N.md + LOOP-PROGRESS Z 段。⏸ owner:zh 随下次送审(bump+归档同 CLI 流)+ ASC zh 商店元数据 + 品牌词否决窗· 前次 synced 2026-08-01(H-loop:**主页状态化 Step1+2 完成** —— 主页开头改为一个状态词 Steady/A bit off/Off your usual/Still waiting,**从下面信号列表已有的 status 数出来、结构上不可能与列表矛盾**;今天还没同步时**整页回退**到最近一份完整的日子并明写是哪天。129/129 单测 + 25/25 UITests。⏸ owner 两个决定:九行指标是否挪出主页、状态块的视觉。**另实证:A3 后台同步真机端到端成功**,但数据 10:30 才到、7-9 点晨报窗口拿到 0 条,晨报需改为按数据到达触发)· 前次 synced 2026-07-31(P6 现场 UI 迭代:range 回答改 modules+notes、荧光笔动画、Apple 分页轮播、Notice 行 Apple-Health 化、图表薄数据分档;并修掉一条**长按被数据重绘清掉**的真 bug —— 详见 §8 同日「手势不是状态」条与 `ripple-core/docs/LOOP-PROGRESS.md` P6 段)· 前次 synced 2026-07-28(**生产库实查**:31 注册 / 19 profile,注册集中在 6-21→**07-18 后零新增**,**过去 7 天 0 人登录**;有真 Apple Watch 数据的仅 2-3 人、数据停在 07-15/16;19 次调查末次 07-21;夜间 cron 仍在跑(llm_call 累计 1492、14 天 13.8 万 token)。→ **上架成功但零留存,瓶颈不在代码**,与 §5.0 GTM 侧 onboarding+UX rework 一致;A3 后台同步未建与零留存是同一个死循环(不开 app→无数据→无 anomaly→无通知→更不开)。**ripple-site 已转正官方宣传站(2026-08-04 owner 定):ripple-health-ai.com + ripple-site-puce.vercel.app,Cloudflare 侧仍在部署,repo=toffemoon/ripple-site**。用户数拆解:31 账号内含 9 个测试号(@example/@test/@ripple.app),**真人约 15**(Apple 7 人 **7/7 全登录成功**;邮箱 24 号里 2 个真 gmail 卡在验证码未验证 + 1 个把域名打成 `gmail.copm`)。**Supabase Pro 当日升级完成**(升级前实证 `backups:[]` 零备份 = 当时最硬的风险;升级后复查 7 份每日备份 COMPLETED、PITR 保持关闭不买 $100/月)。SMTP 已是自定义 Gmail(`gengyue081@gmail.com`,发件人 Ripple)——个人 Gmail 发陌生人验证码的 deliverability 是潜在隐患,未处理。两仓代码自 07-14 build 4 后无功能 commit。历史:07-21 上架实证(iTunes Lookup 07-14 16:46 UTC 发布);S21-S23 三轮拒-修全过,拒因 pattern 见 §8 07-09 条)· 真源 = `ripple-core/docs/LOOP-PROGRESS.md` + `ripple-ios/docs/APP-STORE-SUBMISSION.md`(API 契约见 `ripple-core/docs/CONTRACT.md`) | → `ripple-core/docs/LOOP-PROGRESS.md` · `ripple-core/docs/CONTRACT.md` · `ripple-ios/docs/APP-STORE-SUBMISSION.md` · `~/Desktop/ripple-core` · `~/Desktop/ripple-ios` |
...
| **TemplateApp** | **暂时归档**（2026-08-04）：owner 说可能后续继续开发，复活即移回现役。注意：免费 PG ≈2026-08-12 到期删库（要保数据需在到期前处理）；sub-MD 保留原位 | → [TEMPLATEAPP.md](projects/templateapp/TEMPLATEAPP.md) / [HANDOFF.md](projects/templateapp/HANDOFF.md) |
...
### [2026-08-04] insight: UI 测试写穿持久层就是在给后面的测试布雷 —— 55 个失败、零个指向真凶
Ripple zh 冒烟 UITest 通过**真实 UI 点击**选了「简体中文」——这一步把 `AppleLanguages` 写进了 app 的持久 UserDefaults。类按字母序跑，Chinese* 排最前：后面整个英文套件的 app 启动全变成中文，**55 个断言失败，每一条都写着 "Tab 'Home' not found"，没有一条提到语言**。而这个测试单独跑是绿的——它自己既是污染源又免疫（断言的就是中文），只有全量组合才爆炸。
修法双保险：① 恢复动作**硬断言**（点回「跟随系统」后 assert isSelected——泄漏要在肇事测试里响，不是在五十五个受害者里响）；② 类改名 Z 前缀让它**最后跑**——即使恢复失败也没有下游可污染。
**How to apply**：任何"通过 UI 改持久状态"的测试（语言/主题/账号/开关类设置），先问三件事：**谁在它之后跑（顺序即爆炸半径）、恢复失败会不会无声、单独跑绿能不能证明什么**（第三个答案永远是不能——它是 [2026-08-01]「组合缺陷看屏幕才能抓到」的测试版）。失败分布再补一刀：**几十个失败长得一模一样 ≠ 几十个 bug，= 一个共享状态坏了**——先找共享状态，别逐条修测试。

### [2026-08-04] insight: 本地化第一课 —— 显示格式器的 locale 必须显式注入，「测试恰好跑在英文环境」是隐式依赖
Ripple 中文化 Z2 轮：把 `en_US_POSIX` 显示格式器改成 `.autoupdatingCurrent` 后，两条钉死英文输出的测试立刻红了——**测试进程的 locale 是中文**（输出 "7月31日 星期五"／"周五"），此前全绿只是因为环境碰巧是英文。修法不是查环境为什么变了，而是**把 locale 做成函数参数**（默认 autoupdatingCurrent），测试显式传 `en_US` 断言英文、传 `zh_CN` 断言中文——一次修复同时消灭隐式依赖、又把中文输出钉进了测试。
配套的几条同轮结论：① **POSIX 只属于"造 key 的格式器"**（yyyy-MM-dd 字典键/API 参数），给人看的日期/星期一律 locale 注入；② SwiftUI `Text("字面量")` 天生按 key 查 String Catalog——**视图层字面量零代码改动**，要动代码的只有纯逻辑层拼的 String（`String(localized:)`）；③ 句子模板整句进 catalog 用位置参数（zh 重排语序），**字面 `%` 永远折进参数**不留在 key 里（格式串转义规则是静默坑）；④ 英文默认值=key 本身 ⇒ 现有英文断言测试天然不动，中文走 `-AppleLanguages (zh-Hans)` 冒烟+截图。
**How to apply**：任何"把硬编码英文改本地化"的工程，先立这四条再动手；显示层输出进测试断言时，locale 必须是测试指定的，不是继承的。

### [2026-08-02] insight: 计量器要装在被计量的东西上 —— 护栏放在缓存之前，就是在对免费操作收费
Ripple 主理人报「AI 全断了,too many requests」。当晚证据链:rate_limit 里他一小时 42 次 explain(上限 30)、llm_call **零行**。矛盾的解释:护栏在路由**入口**就计数,而答案缓存在它后面——**重看一个已缓存的答案(零成本、零模型调用)也烧配额**,烧满后真需求全 429。一个"成本护栏"把主理人从自己的产品里锁了出去,锁他的还是不花钱的操作。
修法 = 把护栏下沉到**缓存未命中、真要调模型的那一刻**(ensureBudget 回调,在两个 cache-miss 点各调一次)。30/小时的含义从「30 次请求」变成「30 次真实模型调用」——正常探索碰不到,而重放永远免费,和它的真实成本一致。生产实证:同一主题问两次,计数器只 +1。
**How to apply**:①任何 rate/cost 限制,先问「**它计量的动作和它想保护的成本是不是同一个东西**」——挡在门口最省事,但门内如果有缓存/去重/短路,门口计数必然过收;②诊断侧的对偶(呼应 [2026-07-31] 同一份证据多个原因):**用户侧 429/操作被拒,llm_call/业务表可以完全无痕**——查限流表(rate_limit 的 count vs 上限)是一条独立证据链,别只查业务产出表;③这次定位靠的是主理人一句「错误是 toomanyrequest」——我此前枚举了五种请求形状全 200,因为**护栏是 per-user 状态,换用户复现天然不可能命中**。复现要复现的是状态,不只是形状。

### [2026-08-02] insight: 「多行 = 多份」是个会静默放大一个数量级的默认假设 —— 先问这些行是快照还是切片
Ripple 的日历上，主理人 2026-05-23 的步数显示 **142,193 步**。真实是 20,430。原因不是数据坏了，是**同一天有 11 行，而代码把它们加了起来**（`mean × n`）。那 11 行是 Health Auto Export 一天里多次推送、每次存"今天到此刻的累计"（10,868 → 20,430，`samples_count` 275→543 同步上升）—— **它们互相包含，不是互不重叠的分段**。求和只在"每行是当天的一个切片"时才对。
**它能活 222 个用户-天而没被发现，是因为每一层看起来都没问题**：每一行都是合法读数、`avg/count` 算得也对、测试全绿。**错的只是"把它们加起来"这个动作的含义**——这类错误结构上不可检测，只有理解数据怎么产生的才看得出来。
**更狠的是它写进了两处"契约"**：`day-context.ts` 有注释断言"healthlog_daily 聚合的是互不重叠的小时桶，所以日总量 = mean × n"（假的，那个 rollup 直接对原始行跑 `avg/max/count`）；还有**一条测试把这个 bug 当契约在断言**（`521.44 × 18 = 9386`，注释还写着"这就是日页面显示的数字"）。**一条测试可以像代码一样把缺陷钉死在原地**，而且更难发现——因为它是绿的。
**How to apply**：① 任何"同一个键下有多行"的聚合，动手前先问 **这些行是彼此独立的切片，还是同一个量的连续快照？** 前者求和、后者取最大值，选错就是一个数量级；② 同一个概念在 N 个文件里各写一遍（这里是 5 处 `mean * n`），**修的时候抽成一个共享函数**，否则下次只会修好其中三处；③ **注释和测试都可能在编码错误的假设** —— 发现真相后要就地改掉它们并写明推翻的理由，不要只改代码；④ 优先选**两种情形下都正确**的公式：取 max 对"一天一行的新数据"和"累计快照的老数据"同时成立，这种解不需要判断数据属于哪个年代。

### [2026-08-02] correction: 我把「服务端默认值」当成了「来源标记」，整晚基于它的结论全是错的
夜审我报了一条大发现：「HRV / 静息心率 / 呼吸率**从来没有**到过生产环境——零行、每个用户、所有时间」，还把它当作论据写进了 `HealthKitManager.swift` 的注释。**是假的。** 光 hrv_sdnn 就有约 3385 行、覆盖 5 个用户。
错的是查询不是数据。我按 `source = 'HealthKit'` 分组，当成"这是我们 app 发的"。但 `'HealthKit'` 是**服务端在客户端没传 source 时填的默认值**——只有累计和睡眠那两条分支不传，瞬时指标那条传的是**设备名**（`s.sourceRevision.source.name`，比如"耿越的Apple Watch"）。于是我们 app 发出的每一条瞬时数据，都被我归进了"不是我们 app"。真正的判据是 `extra->>'ingest' = 'native'`。
**它还错了第二次，而且是独立的一次**：schema 里**根本没有东西能区分后台增量和前台回填**——两条路写出的行一模一样。所以"**这条路**从来没送达过"这句话，从这张表里**本来就答不出来**。我用一份识别不了代码路径的证据，去给一条代码路径定罪。
**How to apply**：① 一个字段如果**有服务端默认值**，那它的出现只说明"客户端没传"，**不说明客户端是谁**——拿它当身份标记之前，先去代码里查**哪些分支会传、哪些不会**；② 要给某条代码路径归因，先问"**这张表里有什么东西能把这条路和别的路分开**"，答不出来就不能归因，只能说"整体上没发生"；③ 判据要选**写入方显式打的标记**（这里是 `ingest: 'native'`），不要选**可能被默认值填充**的字段。
**关联**：呼应 [[2026-07-31 绿有四种失真]] —— 这是第五种：**红也可能是假的**，我基于错误分组"发现"了一个根本不存在的 bug，还差点让它指导了后续设计。

### [2026-08-02] insight: 锚点是变更日志里的位置，不是时间下限 —— 任何批量导入都会被整体重放
Ripple 的增量同步用 `HKAnchoredObjectQuery`：有锚点时谓词是 `nil`，因为"锚点之后的就是新数据"。**这个直觉是错的。** 锚点是 HealthKit **变更日志里的位置**，不是日期；任何**新加入库**的东西都算变更，**不管读数本身是哪年的**。于是配对一块旧表、恢复一次备份、iCloud 同步进另一台设备的历史——几千条 2022 年的样本会**追加到锚点之后**，而 nil 谓词不拦任何东西。
生产实证：2026-07-30 13:30 UTC **一分钟内 7315 条**，日期跨 2022-08-03 → 2026-06-20，来自一块主理人不戴的表，占了这个账户原生通道**有史以来全部数据的 92%**。前台回填做不到这件事——它只要 90 天。
**How to apply**：① **凡是"增量/游标/变更流"接口，都要另外加一条按业务时间的下限**，因为游标管的是"我读到哪了"，不是"数据有多新"——两者在批量导入面前会分家；② 判断方法：问"**这个游标前进，是因为有新事件，还是因为有东西被写进来了？**"后者就一定会被历史导入引爆；③ 修的时候要**如实说清楚下限拦不住什么**——我这条下限挡掉 7315 里的 7005 条，剩下 310 条落在 90 天内照样进得来，**日期下限在原理上分不清"谁的"近期数据**。
**同族**：Discord presence 那条（[2026-05-04] 实时-only 无历史）是反面——那边是拿不到历史，这边是历史不请自来。

### [2026-08-02] insight: 摄取边界丢掉单位字段 = 4.18 倍的错误可以永远查不出来
主理人账户里有一天活动能量 10317 kcal（人做不到）。夜审我以为修好了（改成按天聚合而不是按推送聚合），**没有**——部署之后新入库的行照样 10317。真因：`10317 / 4.184 = 2466`，**payload 是千焦**。HAE 每个 metric 都带 `units` 字段，解析器**从来没读过**，所以所有数字都被默认"已经是我们的单位"。
**它能藏住是因为 4.18 倍在结构上完全正常**：一个指标、一天、聚合正确（`day_grouped: true`、三天的 `samples_count` 相同）、还在合理性上限 30000 以内。**唯一能发现它的方式是跟另一条路对照**——原生那条明确写了 `HKUnit.kilocalorie()`，同期是 134 kcal。
**How to apply**：① **任何摄取边界，收到的单位必须显式读取并转换，不能假设对方用你的单位**；② **转换要放在合理性校验之前**——校验该判的是你要存的那个数（顺带救回被静默丢弃的数据：一条按秒声明的 HRV 0.062 落在 `[1,400]` 外，一直被扔掉）；③ **把收到的原始单位存进去**（我加了 `extra.unit_in`），否则下次只能靠数量级反推，而这次就是靠反推才找到的；④ 只在单位**已声明且已识别**为非规范时才换算，没声明或不认识的**原样通过**——猜测比不动更危险。
**通用形态**：凡是"同一个数据库列有多个写入方"（不同 app 版本、不同第三方、不同 SDK），**必须假设它们的单位/口径不一致**，并让每一行自己说明它是什么口径。呼应 [2026-07-31] 「同一件事的两个数字并排出现 = 必然读成矛盾」——那条讲展示层，这条讲存储层：**口径不写下来，早晚会被当成同一个东西相加**。

### [2026-08-01] insight: 一个 guard 会让另一处的文案永远说不出真话 —— 「加防护」有时会把别处变成死代码或谎话
Ripple K2 加了一条防护：证据行只显示「句子点名过的指标」的 case（防止句子说 HRV、底下卡片说血氧）。K3 准备给「空态文案」拍截图时，把条件推了一遍才发现**这条防护让那句文案不可能有正确的时候**：过滤函数第一行是 `guard !offUsual.isEmpty else { return [] }`，而文案的守卫是「仅在 steady 时渲染」——**两者是同一个条件**。于是：① steady 时句子已经写着「9 项读数都在常态范围」，文案是**同一件事说第二遍**；② 而当天若有一条 case 是针对「今天读数正常的指标」（真实存在：demo 数据 7-29 就是血氧有 case、血氧读数正常），文案会说「今天没什么值得标记」而 Notice 里躺着一条 —— **那是假话**。两种情况**穷尽了所有可能**，没有第三种。
**How to apply**：① **加了过滤/守卫之后，要回头检查它让哪些既有文案变成了恒真或恒假** —— 新防护改变的是全局条件，不只是它所在那一处；② 判断一句话该不该留，方法是**枚举它出现的全部条件**，而不是想象它出现时的样子；③ 发现「要么冗余、要么撒谎」这种二分时，删掉比修补更对，因为**修补通常是给它编一个第三种情况**；④ 删掉之后**必须在原地留注释写明推理**——文档里那句「空态应该读起来像好消息」还在，下一个人会很自然地把它加回来，而他不会重新推一遍过滤器的影响。**注释是拦住那次回归的唯一东西。**
**关联**：这条同时是 [[2026-08-01 被授权决策之后，退让也要有理由]] 的正例 —— 推翻三方共识用的是**逻辑等价 + 可达的假话**（实质理由），不是「我觉得更好」。

### [2026-08-02] insight: 用免责声明盖住错误文档 = 留了一个能把用户弄坏的陷阱
Ripple 的 `docs/API.md` 顶部写着「LLM 现在是 OpenAI 独家，**忽略下面提到的 DeepSeek/Kimi/Azure**」，下面照旧写着「provider-agnostic（默认 DeepSeek）」和「`provider ∈ deepseek/kimi/openai`」。有人知道内容错了，**加了一句免责声明而不是改正文**。
**代价是实打实的**：同一夜查到生产库里有 1 个用户的 profile 存着 `provider=deepseek`，而 registry 从 07-02 起只有 `openai` —— 他的每一次 AI 调用都在到达模型之前抛错，`llm_call` 记录了 128 次，然后在 07-14 停止（**大概率不是修好了，是这个人不用了**）。那段 BYO-provider 文档，正是一个用户照着做就会把自己弄成这样的东西。
**How to apply**：① **免责声明不是修复** —— 读者要么没看到那句、要么看到了也不知道具体哪几段该忽略；发现文档错了就改正文，改不动就删掉那段；② 区分要明确：**描述"当前系统是什么"的错了必须改，记录"发生过什么变更"的要保留**（本次保留了 ARCHITECTURE/ROADMAP 里"moved off DeepSeek 2026-07-02"这类历史陈述）；③ **最危险的文档是"照着做会坏"的那种**（配置项、可选值、示例代码），优先级高于描述性段落；④ 顺带的通用防御：**凡是接受用户可选值的地方，值不在白名单里就回退到默认，不要抛错** —— 抛错等于让用户永久失败且无从得知。

### [2026-08-02] insight: 模拟器跑一整夜会退化 —— 判据是"不同测试、相同超时、主机却正常"
夜间 loop 收尾时 iOS 单测挂 1 条:`NSURLErrorTimedOut(-1001)`,卡在 30 秒。重跑 —— **挂的是另一条测试**,同样 -1001、同样 30 秒。而主机 `curl` 同一个后端 **0.3–0.6 秒、三次全 200**。`xcrun simctl shutdown && boot` 之后 **154/0/1 一次通过**。
**判据(比结论有用)**:① **失败的性质** —— 超时不是断言失败;② **失败的分布** —— 每次换一条测试,说明不是某条测试的问题;③ **对照组** —— 主机能在毫秒级到达同一个后端。三条同时成立 = 模拟器网络栈退化,不是代码、不是后端。
**How to apply**:长时间自驱 loop 里,模拟器跑一夜、装十几次 app 之后要把"重启模拟器"当成**常规排查步骤**而不是最后手段;并且**在重启前先跑对照组**(主机 curl),否则重启只是碰运气,拿不到"是模拟器的错"这个结论。呼应 [2026-07-31]「红了但不是你的错」—— 同族的第三种来源:执行环境本身。

### [2026-08-02] insight: 生产的错误日志是最便宜的 bug 来源，而"没人看"是它的默认状态
Ripple 夜间审计：为了追一个不合理的基线值（active_energy 3214 kcal/天）翻开 `error_log`，**自 2026-07-29 起总共只有 81 行，其中 79 行是两个从没有人知道的活跃生产缺陷**——① 一条重复行让整批健康数据被 Postgres 拒收（21000，四天丢 44 批）；② "撤销评分"自上线起 100% 坏（校验器接受 `rating=0`，CHECK 约束只允许 ±1）。剩下 2 行牵出第三个：注销账户没删 `healthlog_daily`（263 个已删用户 9080 行残留 = 隐私缺口），而它每晚把基线重算 cron 撞崩。
**三个缺陷的共同点**：**测试套件全绿、app 里完全看不见、用户不会报**。摄取失败对用户表现为"数据有点少"，撤销评分失败表现为"点了没反应"，cron 崩溃表现为"今天没提醒"。
**How to apply**：① 任何有生产流量的项目，**把"读一遍错误日志"当成固定动作**（周一次即可），它的产出/成本比任何一种测试都高；② 判据不是"日志里有没有 error"，而是**按 `path × status` 聚合看重复模式**——单条像噪音，聚合起来是缺陷；③ **重点看 5xx 里的数据库错误码**：`21000`(批内冲突键) / `23503`(外键，常＝残留孤儿) / `23514`(CHECK，常＝代码改了约束没改)，这三个几乎必然是真 bug 而不是用户输入问题；④ 没有错误日志表的项目，加一张比加任何监控都值。

### [2026-08-02] insight: 测试骗了我四次，每次都是测试错、产品对 —— 判据是"结论与眼睛冲突时先怀疑测试"
一夜之内四次:①`"still water"` 含 `"ill"`（短词**子串匹配**误伤，"bad" 会命中 "badge"）;②断言一个我**自己刚删掉**的元素存在（删功能没删契约）;③`app.descendants` 数到 6 张卡而截图只有 1 张（**TabView 保活**，查询够到了别的 tab）;④限定容器后**仍是 6**——打印才发现是**同一个 identifier 在层级里重复 6 次**（identifier 挂容器时会出现在 button/cell/各层包装上，**按前缀数元素＝数树节点，不是数逻辑行**）。
**四次的共同点：失败信息读起来都像真 bug**（"Home is showing more than two readings" 完全像回归）。唯一每次把我拉回来的是那张只有一张卡的截图。**如果没有截图，我会去改产品代码——修一个不存在的问题，同时把正确行为改坏。**
**How to apply**：① **测试结论与肉眼所见冲突时，先怀疑测试**（默认相反是错的）;② 第 ③④ 两次我各烧掉一轮十几分钟去**猜**原因，第三次改成**把匹配到的东西全打印出来**，一行输出当场结案 —— **猜第二次的时候就该停下来去看真实状态**（呼应 [2026-07-31]「把不可见的状态变成可见的输出，比继续读代码猜快一个数量级」，这次我自己没第一时间照做）;③ XCUITest 数元素一律 **数去重后的 identifier**;④ 短词禁用表一律**按词切分**再比。
**关联**：[[2026-07-31 绿有四种失真]] 说的是"绿了也可能错"，这条补的是第五种 —— **红了，而且红的是测试本身**。

### [2026-08-01] insight: 「提前量」要按**一次完整操作的时长**取，不是按「离悬崖远一点」取
K3 写的 `refresh_fixture_jwt.py` 里我设 `SKEW_SECONDS = 300`（剩不到 5 分钟才续），脑子里想的是"别在悬崖边"。K5 第一次真用它，它报 **"still valid for 20m — not refreshing"** —— 而一次完整门（单测 + UITests）要跑十几分钟，**token 会在套件中途过期**：前面的 live 测试过、后面的 401，结果读起来像偶发而不是像凭据过期。**讽刺的是我在那个常量的注释里就写了这个最坏情况，却把阈值设小了。**
**How to apply**：① 任何「过期/超时/配额」的提前量，问的应该是「**我接下来这一次操作要跑多久**」，而不是「留多少余量算安全」——前者有确定答案，后者是凭感觉；② 目标是让状态只剩**两种**（「够用」和「已续」），中间态（"够开始但不够跑完"）才是真正会骗人的那个；③ **自己写的工具，第一次真用时要看它的输出对不对，而不是看它有没有报错** —— 它当时正常退出、打印了一句合理的话，只有把那句话跟"接下来要花多久"放一起才看得出是错的。
**关联**：[[2026-08-01 同一个环境问题让 loop 交两次税]] 是这条的前一半（做了工具），这条是后一半（**工具本身也要被验证**）。

### [2026-08-01] insight: 同一个环境问题让 loop 交两次税，就该变成工具而不是变成经验
Ripple loop 的 `LiveSmokeTests` 从 `loop-fixtures.json` 读 JWT，而 Supabase access token **约 1 小时过期**。K1 撞上一次（13 条失败横跨 5 个测试类、全是 401，**看起来完全像刚才那次改动的回归**，花了一轮诊断），K3 又撞一次赔了一轮重跑。第二次之后写成 `scripts/refresh_fixture_jwt.py`（用 fixtures 里的 refresh_token，幂等：剩余时间足够就不动，只打印新过期时间不打印密钥）。
**How to apply**：① **判据是「同一个非代码问题第二次消耗一轮」** —— 第一次诊断是必要成本，第二次就是在给流程交税，该做工具了；② 工具要**幂等**，否则它自己会变成每轮的仪式；③ 失败时要**明说正确的下一步并堵死错误的那条**——脚本里写了「refresh token 也过期就重新种用户，**不要跳过 live gate**」，因为跳过会把「没测」变成一个绿勾（[[2026-08-01 零测试覆盖常常是结构问题]] 里「绿了但根本没测」的同族）。

### [2026-08-01] insight: 零测试覆盖常常是**结构问题**不是纪律问题 —— 以及三种缺陷要三种不同动作才能发现
Ripple `SignalGlance`（信号行那句话）**零测试覆盖**，里面躺了 6 个用户可见的缺陷。根因不是没人想写测试：**测试 target 是逐个文件编译 app 源码的**，任何跟 SwiftUI View 同住一个文件的逻辑都会把整个设计系统拖进来、因此进不了 logic-test target。`SignalGlance` 跟 `ExpandableSignalRow` 同文件 ⇒ **物理上测不到** ⇒ 6 个 bug 在那儿活了很久。这个仓库其实早就解决过一次（project.yml 给 `DigestLanguage` 写着"views live separately so this half is testable"），只是没有推广成规则。
**How to apply**：① 看到某个文件"没有测试"，先问**它能不能被测**，再问为什么没人写——纯逻辑跟 View 同文件、跟 IO 同文件、跟单例耦合，都是"测不到"而不是"没写"；② 修 bug 只修了那几条，**把逻辑拆出去才是让下一批 bug 无处生长**，所以拆文件优先级高于修 bug；③ 任何"这块历史上没测过"的地方，默认假设它藏着不止一个缺陷。

**同一轮撞出来的第二层，比第一层更通用 —— 6 个缺陷是被三种不同动作发现的，缺一种就漏一类：**
- **读代码 / 跑测试** → 抓到逻辑错（写死 "today"、7 天趋势当单日形容词）。
- **把不变量写成可执行断言** → 断言自己找出了我没想到的第二处：我立的规则是「行上印的数必须原样出现在句子里」，本意修 sleep quality 的 `0.8`/`80%`，一跑就撞出 steps 的 `9,386`/`9386`。**"把规则写成断言"和"检查已知的那一处"是两件事**，前者会替你搜索同族。
- **把 app 打开看** → 抓到**组合缺陷**：句子末尾印 "Hold for Ripple's full read."，而视图无条件在下一行渲染同一句提示——**9 条单测全程绿**，因为测试读的是**孤立的句子**，这个缺陷只存在于句子与视图的组合里。
**结论**：单测证明不了"两个各自正确的东西放一起是否还正确"。呼应 [2026-07-30]「测试全绿≠做对了」与 [2026-07-31]「绿有四种失真」—— 这条给出**对应的动作**：逻辑错跑测试，同族错写不变量，组合错看屏幕。

**附一条判据（跟 [2026-07-31]「同一份证据可以有多个原因」互为镜像）**：改完代码测试就红，最自然的假设是"我改坏了"，但**判据是失败的分布，不是失败的时间点**。本轮全红实为 `loop-fixtures.json` 里的 JWT 过期 6.5 小时（全部失败集中在一个与改动无关的类、且全是 401）。**红灯紧跟改动出现，这个先后关系本身不构成证据。**

### [2026-08-01] feedback: 被授权决策之后，退让也要有理由 —— 别拿"对方反驳得漂亮"当撤回的依据
owner 原话：「我认为圆形就是不妥，**你以后再决策的时候一定要自信，有足够理由再推翻**」。触发场景：我判定 Ripple 主页状态块**不要用常驻圆形图标**（理由：圆形会被读成仪表 → 等于让已否掉的 0-100 分从后门回来）。Yufei 那边的 AI 反驳"涟漪是品牌隐喻、不是仪表"，我当场认输改口"我把圆形和仪表混为一谈了"，退成给护栏。**owner 判定我退错了**——而且对方 AI 隔天自己也改判回"不要常驻图形"。
**Why 我退得不对**：我手上本来有一个**决定性且不是稻草人**的理由没用出来——**常驻图形是需要图例才能读的物件，而这一页的全部主张就是"词本身即答案"**；Ripple 已经在 Calendar 上付过这笔税（网格下面三行说明文字在教实心点 vs 描边圈），主页顶部再加一套形状词汇 = 要么第二份图例、要么一个没人看得懂的符号。我却在较弱的那条轴（"圆形=仪表"可被反驳为过度推广）上认输了。**"对方的反驳打中了我某一条论据" ≠ "我的结论错了"** —— 正确动作是换用更强的论据，而不是撤回结论。
**How to apply**：① 被授权拍板后，**推翻自己需要跟当初立论同等强度的新理由**，"对方讲得有道理/态度好"不算；② 撤回之前先自查"我最强的那条论据用出来了吗"，没用出来就先补上再谈；③ 真的错了要干脆认（这次决策一我撤回自己的方案就是对的，因为对方指出我的方案在我自己指认的病灶上更差 —— 那是**实质反例**，不是修辞）；④ 区分**实质反例**（能举出我的方案更差的具体场景）与**修辞压力**（听起来更周全），只有前者构成撤回理由。
**关联**：[[2026-08-01 owner 授权 Claude 做 ripple-ios 的 UI 决策]] —— 授权是前提，这条是授权之后怎么守住判断。

### [2026-08-01] decision: owner 授权 Claude 做 ripple-ios 的 UI 决策 —— 2026-07-01 视觉边界在 Ripple 放宽
owner 原话：「你现在更多可以扮演这个项目的决策者」。适用范围 = **ripple-ios 的产品/UI 方向决策**（如 Home 状态页的列表去留、状态视觉形态）——[2026-07-01]「美学微调交人做」的边界**在 Ripple 项目上解除**，其它项目仍按原边界。
**没变的部分**：① 决策必须给理由、owner 随时能否决；② 像素级 craft（间距/字阶/动效曲线）实现后仍须**截图实看**才许说完成，且只说「实现了决策」不自称「好看」；③ 方向级判断我拍板，但最终视觉精修若 owner 想交设计师仍成立。首个行使 = Home 状态页两个 ⏸ 项（九行信号列表折叠 + 状态视觉形态，原 PLAN-HOME-STATUS.md 第 3、4 步）。

### [2026-08-01] insight: 用相对日期还不够 —— 跨月边界会让一整批测试在每月 1 号集体挂
Ripple 的 UITests 在 2026-08-01 **五条同时挂**,信息都是 "data day cell missing"。看起来像我刚改的东西炸了,实际**一行相关代码都没动**:日历打开在**今天所属的月份**,而这些测试点的是"昨天"——每个月有 27-30 天"昨天"和"今天"同月,**只有 1 号那天它在上个月**,于是格子根本不在屏上。
**关键点是它推翻了我之前立的一条规矩还不够。** §8 [2026-07-09] 已经写过"测试里凡是'某天有无数据'的断言必须用相对日期"——这些测试**用的就是相对日期**(`Date()` 减一天),照样挂。因为**算出正确的日期只是一半;另一半是让界面正在看那个日期所属的时间段**。修法 = 一个 helper:找不到格子就翻回上个月再找。
**通用形态:任何"导航到某个日期/时段"的测试,必须同时控制两件事——目标的 key,和视图当前所处的时段。** 只做前者,测试就会在边界日无声地失效。
**更值得记的是它的时间特性:日期边界 bug 是按日程表引爆的**(每月 1 号、午夜、夏令时切换、年末、闰日)。也就是说**今天全绿的测试套件,可能明天因为跟代码毫无关系的原因变红** —— 所以红灯的第一个假设永远不该是"我刚改坏了"(呼应 [2026-07-31]「红了但不是你的错」)。反过来:**跨过这类边界那天,应该主动跑一次全量**,那是唯一能提前抓到它们的时机。

### [2026-07-31] insight: 手势不是状态 —— 一次数据写入就能把 SwiftUI 长按清掉(而且证据看起来像"手势没触发")
Ripple 收尾时一条测试红了:"长按今天必须能问 AI"。**手动按,一按就开** —— 典型的"测试比人苛刻"错觉,我差点判成 XCUITest 怪癖。查到底之后是四层,每层都通用:
1. **`@Observable` 不做 diff。** 把一个**完全相同**的集合再赋值一次,照样通知所有观察者、照样重绘。所以"数据没变"必须**自己判断**(`if merged != old { old = merged }`),不能指望框架帮你省。
2. **重绘会把正在进行的手势清掉。** 后台请求晚到一两秒落地,正好落在手指按着的时候,长按识别器**还没走到阈值就被重置**,于是"按了没反应"。**同一个操作一秒后再做又完全正常** —— 这就是它极难抓的原因。
3. **取消在飞的请求本身会制造竞态。** 翻页时 `task.cancel()` 看着很对(没人看的月份为什么还要加载),实际是:第一次加载被取消 → 翻回来必须重新拉 → 答案就落在用户手指按下去的那一秒。**改成结果并集 + 不取消**,晚到无害,反而更省。
4. **一次未完成的手势会毒害后面所有手势。** 长按一个"不可操作"的对象,那次 `.onEnded` **不会触发**,留下的选择状态被下一次长按的 `guard selection == nil` 撞上,于是**之后每一次长按都被静默挡掉**。新的一次交互**永远应该开始新的状态**,不向遗留状态让路。
**结构性结论:凡是"必须完成才有意义"的交互,不要把它挂在手势的存活上。** 触摸**上报**(`onPressingChanged`)在重绘中是活着的,把计时挂在上报上;手势只保留它独有的能力(跟着手指移动)。
**方法论(比结论更值钱)**:我先猜了两个原因,都错。真正解决问题的是**把内部状态画到界面上**(一个临时 Text 显示 `pressedDay`/`selection`),让失败时的 UI 层级 dump 里能读到 —— 立刻看到"按下的日子记对了、选择却是空的",一句话把"按错格子"和"识别器没走完"分开。**UI 层的疑难杂症,想办法让不可见的状态变成可见的输出,比继续读代码猜快一个数量级。**
另一条一起记的判据:**`sleep 3` 过、`sleep 1` 挂 = 竞态,不是逻辑错。** 拿到这个之前所有推理都是空转;拿到之后方向立刻收敛。修完**必须把 sleep 全删掉**,不留"靠等待通过"的测试。
呼应 [2026-07-31]「绿有四种失真」—— 这是第三种(红了但不是你想的那个原因)的完整案例。
...
### [2026-07-31] insight: 「绿」有四种失真 —— 三种只有看真东西才能发现
Ripple build5 一夜(L1-L6)把四种都遇齐了,值得当检查表:
① **绿了但做错了**(测试通过、行为是错的)—— 如 deviation 标签错:三个数并排,每个单看都对。
② **绿了但根本没测** —— UITest 里 `if x.exists { 断言 }`,条件不成立整段跳过、照样绿。等于写了一条永远不会失败的测试还以为它在保护契约。
③ **红了但不是你的错** —— explain 失败、后端零 LLM 调用,我照旧规则判"手势没触发",**实际是 429 限流**(请求在到达模型前被拒,同样不留记录)。
④ **绿了、也真跑了、但功能被 catch 悄悄关掉了** —— 给端点加 `delta` 字段,取时区那句查询 400 被自己的降级 catch 吞掉,**所有行都没有 delta、端点 200、测试全绿**。
**②③④ 都无法靠跑测试发现**,只能靠:看截图 / 上真机 / 打线上接口读 JSON / 查后端表。今晚每一个重要发现无一例外来自这四样。
**最后一次尤其说明问题**:L5 的框选手势,测试挂了,我完全可以判成"XCUITest 合成按压不可靠"糊弄过去 —— **手动去模拟器按了一下,发现按住不动真的毫无反应**,是真实功能回归,差点送进审核。
**How to apply**:(1) 每轮至少一次**看图**,并在日志写下看到了什么;(2) 测试失败时**先看用户看到的那一屏**,再去查后端表(我一度顺序反了);(3) 带 fallback 的功能,验证必须是"**它真的生效了**"而不是"它没崩";(4) `if x.exists` 之类的条件断言,**必须让"跳过"在日志里出声**。

### [2026-07-31] insight: 新手势不要和签名手势抢同一片像素 —— 宁可加一个显式模式
Ripple 会上要"日历上长按+滑动框选一段时间给 AI 分析"。按原样实现了,**能拖、拖通了**,但完整测试挂 —— 手动验证发现**按住不动完全没反应**:`LongPressGesture.sequenced(before: DragGesture)` 对**完全静止的手指不产生位置事件**,选区从没建立(真人手指微动掩盖了它;静止按压、辅助输入、测试全失效)。
而这片像素上已经有 app 的**签名交互**(按住某天问 AI,新手引导教的就是它)。两个长按识别器抢同一区域,正是同一晚早些时候把整个日历弄坏的原因(叠了个 0 距离 DragGesture,点击和长按一起被吃)。
**处理**:框选改为进入**显式模式**(一个「Select days」按钮),模式内裸拖即可、无任何竞争手势,模式外原有交互**完全不动**。代价一次点击。并且**同时提供预设(本周/本月)作为无障碍路径** —— 限时拖拽在 VoiceOver 或运动障碍下根本不可执行。
**How to apply**:(1) 想在**已有手势的区域**加新手势时,先问"能不能用一个显式模式把它们分开",而不是想办法让两个识别器共存;(2) 覆盖/复用一个**已上线且被引导教过**的手势,默认是错的,除非新语义严格更优;(3) 任何**限时/拖拽**类交互都必须有一条**非手势的等价路径**,这既是无障碍也是可测性;(4) 手势改动**必须手动验证"什么都不做"的那个 case**(按住不动、点一下就松、拖到界外),它们最容易被合成事件掩盖。

### [2026-07-31] insight: 配色方案要先量亮度 —— 语义色阶必须单调,否则去掉颜色就反了
Ripple 评审会定"日历绿→红,越红越有问题,像 GitHub 那种深浅色"。动手前把现有色板的**相对亮度**算出来:青绿 `#37C2BA` = **163.9**、琥珀 `#E6B45A` = **184.1**、红 `#FF6B5E` = **137.5** —— **非单调,中间档最亮**。也就是说去掉色相后(色盲、强光、灰度打印),"中等严重"看起来比"最严重"更重,**语义整个反过来**。而**同一色相变强度**是单调的(40→64→92→121→138)。**GitHub 贡献图变的正是一个绿的深浅,不是绿黄红换色相** —— 会上那句"像 GitHub 那种深浅色","深浅"才是关键词。
**How to apply**:(1) 任何"用颜色表达程度"的需求,**先算亮度序列**(`0.2126R+0.7152G+0.0722B`),不单调就别用;(2) 优先**单色相 × 强度**,它天生单调且不需要新增色板 token;(3) **验收要可执行**:把截图转纯灰度,仍能分辨程度才算过(比"我觉得能看清"强得多);(4) 颜色之外**保留第二通道**(形状/图案/文字)——Ripple 保留了实心点 vs 描边环。
呼应 [2026-07-30] E3「两个 5pt 圆点只靠红绿区分,约 8% 男性读不出来」—— 同一条原则的量化版本。

### [2026-07-31] insight: 加"新 AI 功能"时,扩展现有端点 vs 新开端点 = 合规面数量之差
Ripple 要做"框选一段时间 → AI 分析"。直觉是新开一个 `/v1/range-analysis`。**没这么做**,而是给现有 `/v1/explain` 加了一个 `subject_type: 'range'`。理由不是省代码,是**合规面**:把健康数据发给 LLM 的**每一个新端点都是一个全新的 AI 面**,按 §8 [2026-07-09] 的已知拒因,它需要自己的同意门、同意页逐项枚举、cost guard、1.4.1 引用、以及客户端一套呈现语言。扩展现有端点则**全部继承**——一扇门,一处会漂移的地方。
**How to apply**:(1) 新 AI 能力先问"**它能不能表达成现有端点的一个新主体/新参数**";(2) 只有当**数据流真的不同**(发送的数据类别变了、接收方变了)才值得新开端点,因为那时同意枚举本来就要改;(3) 用户输入的自由文本进 prompt 时,**以"用户说了什么"的形式引用**(quoted as content),让打进去的指令读作内容而非命令,并硬性限长。

### [2026-07-31] insight: 派生 > 存储 —— 会被迟到数据改写的东西不要建表冻结
Ripple 要给 anomaly 加"这个问题横跨哪几天"。直觉是加两列 + 迁移 + 回填。**没这么做**:边界是**从日聚合序列推出来的**(从触发日向两边走,直到某天回到正常带内)。理由:迁移会把边界**冻结成一次猜测**,而一条迟到的健康同步补上中间某天之后,存下来的区间就永久错了;派生则随数据到达自动保持正确,且**不需要回填**。
**判据**:这个值是「**事实**」还是「**当前数据的函数**」?是后者就派生。配套细节:**缺失的一天算作区间结束**(没记录 ≠ 问题在延续),两端加上限防止脏数据把遍历拖死。

### [2026-07-31] insight: 降级用的 catch 会把「功能没生效」伪装成「功能正常」
Ripple L2 给 `/v1/anomalies` 加结构化 `delta` 时,取用户时区那一版用了 `selectForUser('profiles', …)` —— 而 **`profiles` 表是按 `id` 建的、不是 `user_id`**,全库唯一不适配那个 helper 的用户表。查询 400 被我自己写的 `try/catch`(本意是"取不到就降级返回原始行,别让缺个数字害用户看不到列表")**静默吞掉**:所有行都没有 delta、端点 200、**测试全绿**。是**打真实接口读 JSON** 才发现的。
**How to apply**:(1) 凡是带 fallback 的增强功能,验证必须是"**它真的生效了**"(返回里有那个字段、值对),而不是"**它没崩**";(2) 降级路径至少要 `console.warn` 出真实错误(我写了,但没人看日志——所以更要靠响应验证);(3) 写完新端点字段,**第一件事是用真凭据打一次线上、把返回打印出来**,而不是等 UI 显示不出来再回头查。
呼应 [2026-07-30]「测试全绿≠做对了」·[2026-07-31]「绿了也可能根本没测」·「红了也可能不是你的错」—— 这是第四种失真:**绿了、也真跑了、但功能被 catch 悄悄关掉了。**

### [2026-07-31] insight: 同一件事的两个数字并排出现 = 必然读成矛盾(即使都对)
Ripple 三轮里同一族问题出现三次:①L1 当日页「那天 34 / 你的常态 46.19 / 差异 −13.4%」——第三个数是别的口径(7 天均值 vs 基线);②E5 图表 caption 印 `37.7–52.1` 而下面清单印 `38–52`(同一条带子两种取整,**是我自己引入的第二种**);③L2 通知行徽章 `↓17.6%` 紧挨着句子 `~25% below your usual`(徽章=那天 vs 常态现算,句子=watchdog 触发时量的另一种比较)。**三次每个数字单看都对,并排就是自相矛盾。**
**How to apply**:(1) **一个数字只有唯一出处**;要在第二处提它,要么复述同一个变量,要么**根本别提**(E5 的 caption 改成只解释带子含义、不复述数值);(2) 无法合并时,**给每个数标出它在跟谁比**(L2 徽章加 "vs usual");(3) **优先修到源头**而不是在 UI 上打补丁 —— L2 真正的修法是改 watchdog prompt **禁止句子里出现数字**("行拥有数字,句子拥有故事"),而不是去改渲染;(4) 这类问题**测试基本抓不到**(每个数各自都通过校验),**只有把两处摆在一起看才会暴露** —— 所以每轮必须看截图。

### [2026-07-31] correction: 「没有 LLM 调用 ⇒ 手势没触发」是错的 —— 同一份证据可以有多个原因
我在 Ripple E5 立过一条诊断规则并写进了记忆:UITest 里 AI 没出结果时,查后端 `llm_call`,**没有调用记录就说明手势没触发**(而不是模型慢)。当晚它是对的。几小时后 P1 轮又遇到同样的现象(两次 explain "answer never rendered" + 零 llm_call),我照这条规则判定"又是手势问题" —— **判错了**。去模拟器手动复现:**手势完全正常、sheet 正常打开,里面写着 "Too many requests"**。真因是**限流**(demo 账号一小时内 explain 34 次 > 上限 30),而 **429 在到达模型之前就被拒,同样不留 llm_call 记录**。
**教训(比这条规则本身重要)**:我把一条"某次成立的判据"提升成了通用规则,却没问「**还有什么别的原因会产生一模一样的证据?**」。零调用至少对应三种情况:①请求没发出(手势/前端)②请求被拒(限流/鉴权/校验)③服务端早退。**只有看屏幕上真实显示了什么,才能分辨。**
**How to apply**:(1) 立诊断规则时,显式列出「同样证据的其它可能原因」,列不出来就不要把它当规则;(2) 排查顺序应该是**先看用户看到的那一屏**,再去查后端表——我反了;(3) **环境拒绝 ≠ 代码回归**:测试遇到限流应 `XCTSkip` 并明说"配额用尽、app 没问题",既不误报也不静默吞掉。
呼应 [2026-07-30]「测试全绿≠做对了」与 [2026-07-31]「绿了也可能根本没测」—— 这条是第三种失真:**红了也可能不是你的错**。
...
- **真机跑一遍** → iOS SSE 客户端**零事件**:解析器把「派发」只绑在空行上,而 `URLSession.bytes.lines` 不可靠地给出空行分隔。改成"下一个 `event:` 到来也 flush + `finish()` 收尾"。
- **看截图** → 裸字段 `HRV_anomaly` 直接渲染给用户;home 页被掏空三分之二;**同一个 baseline band 在一屏上印了两遍还对不上**(37.7–52.1 vs 38–52);"15 days of readings over the last 14"(硬编码窗口被自己的数字推翻);退化 band 印成 `(15–15)` 而图表早就拒绝画它。
- **查后端有没有真调用** → UITest 里长按失败,我没假设是自己改坏了,去查 `llm_call`:**那两次根本没产生任何 LLM 调用**,于是"手势没触发"和"模型太慢"被区分开。真因是行**存在**但落在悬浮 tab bar 底下,`press` 打在 tab bar 上。**`exists` 不等于 `isHittable`**;而且"滚一下就可达"这个结论对**阅读**成立、对**按压**不成立。
**How to apply**:任何"UI/流式/呈现"类工作,验证门至少要包含**一次真机或模拟器截图实看** + **一次生产时序或真实调用记录核查**,不能只有 build+test。写进 loop 纪律的话就是一句:**"不只看 green,要看图"**。另一半同样重要 —— 失败时**先去查真实状态再改代码**,别默认是自己最近那次改动坏的。
呼应 [2026-07-01]「没查真实状态不许说健康」、[2026-05-28]「CI green 才算修完」、[2026-07-28]「以可观测的最终状态为准」—— 同一条原则的第四个变体。

### [2026-07-30] insight: 一致性问题只有把两处呈现摆在一起看才会暴露(E5 三连)
E5 做「AI 呈现语言统一」时,**三个不一致全都是把两处并排看才发现的**,单看任一处都合理:①confidence 有两套实现(一个命名组件 + 一个手搓副本),都用光秃秃的 "confidence" 当标签 —— 那是在说这个指标叫什么,不是它意味着什么;②弃权句只有 Explain 有,调查页可以 resolve 成 inconclusive 然后一个字不说;③免责在最像临床报告的那一屏(verdict + 置信度 + 检查时间线)完全缺席,别处又各自换说法。
**最值得记的一条**:让图表变得不用看坐标轴就能读懂的那句白话,**早就写好了 —— 作为 `a11ySummary`,只对 VoiceOver 说**。看得见图的人拿不到它。**a11y 文案常常是产品里写得最清楚的一段话,值得反向检查有没有"只给辅助技术、没给所有人"的好内容。**
**How to apply**:做"统一呈现"类任务,方法是**列出所有说同一件事的地方,把它们的实际输出抄到一起对比**(而不是读代码判断"应该一致")。修的时候优先"同一个数字只有唯一出处"——我自己就在 caption 里引入了 band 的第二种取整,正确修法不是统一取整,而是让 caption 只解释含义、不复述数字。

### [2026-07-30] insight: 权限分类器挂掉时,自驱 loop 的正确姿势(只读+Edit 还活着,变更型工具全死)
Ripple E3 轮实测:`claude-sonnet-5 ... auto mode cannot determine the safety of <tool>` —— 权限分类器服务不可用时,**凡是需要新分类的变更型调用一律被拒**(`xcodebuild` / `xcodegen` / git / 连 MCP 的 build 工具都一样),而**只读 Bash(grep、`date`)与 Edit/Write 照常能用**;此前在本 session 已被分类过的**完全相同的命令字符串**也还能过 —— 所以现象是"有的命令能跑有的不能",很容易误判成命令本身有问题。
**How to apply**:(1) 判定方法 = 换一个从没跑过的变更型命令探一下,若同样报 classifier 不可用,就是服务侧问题,**不要改命令去绕**(改写法只会生成新的待分类字符串,照样被拒);(2) **绝不把"没验证"说成"做完了"** —— 编译/测试/commit 全部做不了的那一轮,该轮就不算完成,日志里明写"代码已落盘、验证门被工具链阻断、本轮不 claim",并把"下轮第一件事 = 补跑验证门"写进交接;(3) 这段时间**只推进零风险工作**(设计文档、计划细化、代码自审读),**不要把未验证的改动继续往上叠** —— 叠越多,分类器恢复后一次性暴雷的面越大;(4) 代码自审读不是白做:E3 这轮正是靠通读改动文件,抓到 E1 留下的真回归(`first_read_shown` 漏斗事件挂在**已不渲染该卡片**的页面上开火)。
**Why 写 §8**:跨项目通用 —— 任何 autonomous loop 都会遇到"验证基础设施短暂不可用",而 loop 的默认压力是"继续产出",最容易滑向"没验证也说完成"。呼应 [2026-07-01] "没查真实状态不许说健康" 与 [2026-05-28] "CI green 才算修完":同一条原则的第三个变体 —— **验证做不到时,唯一诚实的输出是明确说没验证**。
