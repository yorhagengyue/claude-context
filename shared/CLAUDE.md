...
| **Ripple (core + iOS)** | **🚀 v1.0 (1) 已提交 App Store 审核(2026-07-02 晚,自动发布,等 24-48h)** · 后端 live · LLM=OpenAI `gpt-5.4-mini` 独家(不进中国)· 法务页已托管 | NAISC 后重写的多租户 wellness agent。后端 `ripple-core`（Hono+Vercel+Supabase，[ripple-core.vercel.app](https://ripple-core.vercel.app)，结构化 explain + 答案缓存 + investigation agent + softAuth 安全默认、~147 测试全绿）。原生 iOS `ripple-ios`（SwiftUI/iOS26/XcodeGen）**已从旧"仪表盘"彻底重做成愿景「调查中心 / shows-its-work」**(session-only cron 自驱 loop R0→R22+,G1-G4 四门):日历 Home + 长按 Explain Sheet(唯一 AI 入口,充能环+打字机)+ 调查三态 + anomaly 通知 + facts/intake + 设置读写;**联动**=分析关联 events+lifestyle facts;legacy chatbot/dashboard 全删。**原生 Apple 登录已接**(e5f9764,⏸ 待核对 Supabase Authorized Client IDs 含 `com.ripplehealth.ios`)。旧 web 前端 `ripple`(含 peer)已**暂停用**。⚠️ 真源 = `ripple-core/docs/LOOP-PROGRESS.md`+`PLAN-V1-FRONTEND.md`+ ripple **SKILL**；**ASC 提交记录 = `ripple-ios/docs/APP-STORE-SUBMISSION.md` "ASC 提交记录"段(每页实际选项全存档)**;法务页 `https://ripple-legal.vercel.app`(源 `~/Desktop/ripple-legal/`);**凭据已全部轮换并验证(S16/S17,旧钥 401 死透)**。**S-loop 上架冲刺已完结(2026-07-02 05:03-06:36,S1-S4,主理人预授权结束)**:demo 富数据 35 天 persona+免发码固定 OTP v2(零邮件)/ 提示词 v6(跨日全指标 digest+可选非医疗建议+可读名,真长按验收全过)/ /v1/day 聚合+cumulative baseline 日总量语义(迁移 0021)/ 空日期 UI 修复 / glance 反馈真落库(迁移 0022)/ iCloud codesign 根治;凭证 npm 145/145 + iOS 42+7+2 全绿 + 匿名 401 复验。**剩余全部 owner-only**(轮换凭据/ASC/真机 Apple 登录/归档提交,见 APP-STORE-SUBMISSION.md 🔴 段);终局记录 = LOOP-PROGRESS.md S-loop 终局段。**Post-loop S5-S13(2026-07-02 下午,主理人逐轮直接指令)**:Explain 屏思考式编排+三形态证据图表(v7 图表数据源修 stale-200 bug)/ 首次长按 coach 教学两拍 / Home 改选日内联面板+RIPPLE NOTICED 区块+hasData 微标+点标 tz 修复 / signals tap 一句话本地解读 / 案卡直达既有调查(不重烧 LLM)/ 问答快捷 chips(ask_user options 双形态)/ inbox 已读持久化 / 时间线单相化 / 全 app 统一循环 logo 加载 / **deployment target 降 18.0**(玻璃不受影响,SDK/运行时驱动)/ ASC 套图 8 张 1320×2868。全程每轮 R 格式 journal + 测试门全绿(iOS 54 项 + 后端 targeted)。**终局 S14-S20(2026-07-02 晚)**:长按日期修复(SwiftUI 手势宿主坑:Button/NavLink 吞 hold,全部去 Button 化+钉死测试)/ 同意门审计(NOTICED/inbox 旁路封堵)/ 凭据全轮换(含 legacy JWT 键要单独停用的坑)/ 10 张 6.5" 截图 / NSHealthUpdateUsageDescription 修上传校验 / CLI 归档上传一次过 / ASC 五页全填(隐私 5 数据类型/年龄 9+/DSA 非交易者/医疗设备=否/取消 Mac+VisionPro)→ **添加以供审核已提交**。被拒应对=SUBMIT-WALKTHROUGH 第 19 条;审核期间可补 TestFlight 真机验 Apple 登录 | → `ripple-core/docs/LOOP-PROGRESS.md` · `PLAN-V1-FRONTEND.md` · `ripple-ios/docs/APP-STORE-SUBMISSION.md` · ripple SKILL · `~/Desktop/ripple-core` · `~/Desktop/ripple-ios` |
