...
| ⬜ **未指派** | Inception · conversion-site · short-video · MoyuanIdea（04-03 后零更新，疑似休眠但从未正式宣告） · **StudyOS**（别人的项目；2026-09-18 PR #2 已复核合并，**当前无待办**，下一个需求由时浩然提） |
...
| **StudyOS** | **别人的项目** · 基础版已交付、两台机器真机跑通 · **2026-09-18 PR #2 已复核合并（merge `6ff98b3`），当前无未决 PR、无已知阻塞项**；球在时浩然那边 | 耿越给**时浩然**做的个人学习系统（**学习者 ≠ 制作人**；四门悉尼大学课 ACCT2011/2019 + IBUS2020/2105 **不得当成 owner 本人学籍**，对应 record 仓 DRAWER `c-9f1e3002`）。两部分：Agent 端直接在仓库里工作 + 本机网页端 `127.0.0.1:8765` 浏览/刷新 Canvas 资料（**不含 AI 对话**）。纯 Python 标准库零依赖 · Canvas 只读（不提交/不答题/不签到/不读成绩）· `student/` 同步永不改写 · **改动一律走 PR，只有耿越能合并**。实跑 246–247 文件 / 142 页面 / 338MB，未取得的 13 文件+6 处列表限制如实标注。**本机未 clone** | → [STUDY-OS.md](projects/study-os/STUDY-OS.md) |
| **Slay the Spire 2 AI** | 半成品 · **2026-08-28 headless 线已在 `~/slay_the_spire_runtime/` 复活**（iCloud 抽空后重建；LLM 首次真正参与决策，见 §8 同日条） | PPO + 遗传超参数进化，离自主打游戏还有距离；PPO 训练线源码随 iCloud eviction 丢失待抢救 | 本地 ~/Desktop/slay_the_spire（残）+ ~/slay_the_spire_runtime（活） |
...
**对后续 AI 的指示**：看到 [2026-08-24] 条不要再据此排期 iOS 推送线。当前唯一活的入口是 [STUDY-OS.md](projects/study-os/STUDY-OS.md)；该项目当前**没有待办**——[PR #2](https://github.com/yorhagengyue/study-os/pull/2) 已于同日经独立复核后合并（merge `6ff98b3`），球在时浩然那边。
...
**2. 抢救与重启（已完成，全链路验证）**：live 自动打牌线的 12 个 .py + automation.pyc + sts2-cli 的 git pack+index 完好 → 在 iCloud 外重建 **`~/slay_the_spire_runtime/`**：git checkout-index 恢复 sts2-cli 全部 132 文件、dotnet build 0 错、手写重建五个丢失模块（io / mac_window / review / foursapi_client / 各 __init__，API 从调用点反推）、fresh venv、key 取自 Hermes config 的 4sapi.com 端点、`src/lib` symlink 修 dll 运行时解析。
**3. 两个从未暴露的真 bug（连环）**：① `headless_autoplay.py` 决策超时写死 `min(1.2, --llm-hard-timeout-sec)`——远程 API 永远赢不了 1.2s，**历史 2655 步 llm_used=0，"AI 自动打牌"从头到尾是规则 fallback**（与 Ripple「SSE 从未真正流过」同款：功能被静默降级、一切看着正常）。② 修掉钳制后暴露第二环：LLM 按状态里手牌的 `index` 字段名回参数，translator 只认 `card_index` → LLM 动作全判 invalid 继续 fallback；加 arg alias 归一化后 20/20 步 reason=llm。**教训（验证家族补一条）**：带 fallback 的 AI 功能，验收必须数「LLM 决策真的被执行了几次」，不是看局面在推进。
**现状**：runtime 副本自主运行（claude-sonnet-5 实际决策），dashboard = streamlit :8501。iCloud 原目录未做任何修改。

### [2026-08-26] insight: 真设备上的演示程序必须可暂停 —— 自动循环 + 触觉是缺陷不是功能
SUSS GIF 现场前给 owner 的 Apple Watch 写 look-in 接收端 demo，第一版做成自驱状态机（每 2.5s 自动「到达」+ `play(.notification)` 震动 → 3s 回安静 → 1.8s 再来）。模拟器里截图很好看，**戴到真手腕上就是一直弹、一直震**，owner 出门前才发现（原话「打开的时候一直弹窗 不断的 你应该预防」）。
**教训**：演示程序的默认形态是「**什么都不做，直到被点**」。自动推进不只是骚扰，它还让讲解者无法跟台上的节奏对拍——改成纯点按驱动（一点一步一震、零定时器）后反而更好用：手机那边点 Send 的同时点一下表，到达卡与讲解同步。
...
**竞品格局（同日调研）**：官方 Canvas Student app（[instructure/canvas-ios](https://github.com/instructure/canvas-ios) **AGPL 开源**，实现细节可直接参考）已覆盖公告/作业推送，功能对标它没胜算；USYD 自己的 "Sydney Uni" app 是 **Blackboard 白牌壳**（`com.blackboard.android.central.sydney_au`），只做课表/地图/预约、**不碰 Canvas 课程内容** ⇒ 两者之间有缝。第三方生态**主战场在浏览器插件不在 app**：BetterCanvas→改名 **BetterCampus**（因 Canvas 商标顾虑改名，宣称 150 万学生用，做 GPA/planner/AI 学习工具）、Tasks for Canvas（to-do 侧栏）。移动端第三方极少且都很小：[kanbasu](https://github.com/untitled-group/kanbasu)（SJTU 定制、Flutter、19 stars、无推送）、HLSGrades（Flutter，为自建 Canvas 实例做，因官方 app 不支持 self-hosted）。**差异化机会**：官方通知有公认的可靠性缺陷（队列高峰延迟数小时、邮箱进 suppression list、课程 publish 前发的公告永不触发通知），且没人把「Canvas 内容 + 校园服务」缝在一起。
**⇒ 项目定名 Ripple Canvas，计划已批准施工中（2026-08-24）**。定位 = 把 Ripple 的日历/长按问 AI/温柔推送三支柱搬到 Canvas 学业信息上；不做"更好的 Canvas app"（撞官方+商标），做"不漏消息的课程提醒器 + 日程规划器"。**决策**：新姊妹仓 `ripple-canvas-ios`+`ripple-canvas-core`（本地 `~/Desktop/`，从 Ripple 拷复用件；git/GitHub 待 owner 建仓）· 用 Ripple 付费 Apple 账号+TestFlight · 阶段 A 首发日历+公告推送两块一起 · 只 iOS SwiftUI。**完整计划真源** = `~/.claude/plans/ancient-growing-firefly.md`（含 3 个 Explore agent 实证的 iOS/后端逐文件复用映射）。**合规红线**：手工 token 只合规于单用户/演示（阶段 A）；多用户上线必须 OAuth，而 OAuth 的 developer key 只有 USYD IT 能签发——早跟需求方讲。**施工进度**：后端 M0 done（ripple-canvas-core 从 ripple-core 拷基础设施簇+剥健康件，typecheck 净过，新写 `src/lib/canvas.ts` 规范化 activity_stream+planner 两聚合流→实测真 token 拉到 103 条去重条目、4 单测过）。后端可复用真源：APNs 发送器 `apns.ts`、`event` 表(migration 0012)、watchdog 去重模式、cost-guard、pg_cron。⚠️ **摄取反转**（Ripple 客户端推 HealthKit，Canvas 要 cron 拉）+ **Canvas token 必须可解密存**（用 crypto.ts，非 Ripple 的哈希-only device token）。逐步真源以后写进 sub-MD 或项目 repo，暂以 plan 文件为准。**iOS M0 亦 done（同日）**：`ripple-canvas-ios`（XcodeGen，bundle `app.ripplecanvas.ios`，名 Ripple Canvas）——选择性重建而非全拷（app 侧耦合深：RippleAPI 带健康端点、AppState 连 HealthKit、Calendar 依赖健康 DTO），只搬真正领域无关件（DesignSystem 7 文件 / ListKit / RangeSelection / KeychainStore），Canvas 胶水新写；**本地优先架构**：app 先用 Keychain 里的 token **原生直连 Canvas**（URLSession 无 CORS）→ 零后端就能看真数据，等云基础设施起来再切后端代理+推送（CanvasItem 形状与后端规范化器保持一致，切换不动上层）。BUILD SUCCEEDED + 模拟器实装实看三屏（连接屏/Updates/Schedule/Me）真数据全出。
**M2「提醒形态」done（2026-08-24 同日，owner 两次改向后定稿）**：owner 先要"多做交互和细节"，看到搬 Ripple 长按的方案后否掉——**「不要做长按 更多是往提醒方向靠 思考形式」**。于是先去**量**了到底有多少东西值得提醒，结果推翻设计前提：**整学期四门真课只有 5 个带截止日的作业（4 个已过期），剩下会到期的只有 2 个，≥20 分的只有 1 个（Feasibility Report 100 分 9/6）；而未读公告 89 条，其中 86 条来自 11 个广播站**（真课 60 天只发 3 条）。⇒ **列表是错的形式**（列表适合多选一；这里是"一件事别漏，但被 86 条广播埋着"）。定的形式 = **一个数字，且不该需要打开 app 才看得到**：倒计时主页（一件事占整屏，四档紧迫度 on track/coming up/due soon/due today）+ **主屏 WidgetKit 小组件**（官方 Canvas app 没有 deadline widget，这是差异化落点）。已交付：Next up 主页 / 单元勾选 onboarding（按 `^[A-Z]{4}\d{4}` 预勾选，实测正好命中那 4 门）/ Updates 分「你的单元 vs Broadcast（折叠）」+ 未读徽标只数真单元 / 公告+作业详情页 / widget 四档+两种空态 / 17 单测过 / 每屏截图实看。owner 决策：AI 用本地证据层不调 LLM（这轮实际没做 AI）、**本地通知不做等真推送**、只提醒截止日不做"该动手了"提前量。**明确不做且写进计划**：月历网格（一个月 1 个死线的月历截图好看没用，改按天议程）、AI 同意页（没数据出境弹同意页是撒谎）、通知设置屏（通知没做就别放假开关——ripple-core 有过删安慰剂开关的审计）。
**四条本轮教训**：① **widget 别靠主屏手势验证**——iOS 主屏的合成 tap 会穿透 context menu 打到底下图标（连开两次 Watch app），改成**把 widget 视图编进 app、加 `RIPPLE_SCREEN=widget` 调试板按真实点尺寸（158/338×158）渲染全部档位+空态**，一张截图验完，且长久可复用。② **App Group 端到端要查容器文件**：`simctl get_app_container <udid> <bundle> groups` → 解 `Library/Preferences/<group>.plist` 里的快照，确认真写进去了（实测拿到 Feasibility Report/IBUS2105/100pts），不能只看代码写了 `UserDefaults(suiteName:)`。③ **同一事实不许在一张卡上出现两次**：第一版 hero 是「14 DAYS LEFT」压着大字「14」——截图才看出来；改成 kicker 放**紧迫度词**（同时是颜色的第二通道）、数字只有一处，并写了测试钉住。④ **Swift 类型检查器会因链式 filter/compactMap/sorted 超时**（"unable to type-check in reasonable time" 是编译失败不是风格问题）——拆成显式步骤；Ripple 的日历也因此把网格拆出 body。
**两条施工教训（都值得复用）**：① **iOS 模拟器面板的 tap 坐标是「点」不是截图像素**（面板会明写 402x874 points，而截图 ~919px 宽，差 2.29 倍）——我按像素点全部越界落空，且**界面 onAppear 自动聚焦输入框让文字照样输进去了**，制造出"点击生效了"的假象；判据：点了没反应先怀疑坐标空间，别先怀疑代码。② **Canvas 单页硬上限 50 条，per_page 给多少都没用，必须跟 `Link: rel="next"` 翻页**——后端客户端翻了页拿 104 条，iOS 我写成只取一页只拿到 53 条，**两端同一份数据对不上才暴露**（app 单看像正常，53 条也是一屏刷不完的）。凡"同一数据两处实现"，收尾必须把两个数摆在一起对；数字对不上不许放过。

### [2026-08-17] insight: Claude 内置 Browser pane 会被 SSE 长流噎死——本地 web app "全挂死"先怀疑 pane 再怀疑 app
用无头 Chrome 走查 obsession UI 时实证：同一个本地 app（带 2 条 SSE 长连接），在 Claude Code 内置浏览器 pane 里表现为**点击后所有同源 fetch 永久 pending、刷新页面 300s 超时、关 tab 后 9 条僵尸连接不释放**；而 curl 毫秒级正常、干净 Chromium（headless + CDP）里同样的页面同样的请求 7–62ms 全通。结论：pane 的网络拦截层扛不住 event-stream 无限响应，把后续响应全部扣住——**这不是被测 app 的 bug**。
**How to apply**：测带 SSE/长轮询的本地 web app，症状是"第一屏正常、之后全挂"时，先用 curl 对照；要真实点击就起无头 Chrome（`--headless=new --user-data-dir=<临时> --remote-debugging-port`）+ Node 22 自带 WebSocket 写 CDP 驱动（Input.dispatchMouseEvent / Page.captureScreenshot / Runtime.evaluate），几十行就够。别把 pane 的假死写成 app 缺陷——差一点就误报了。
...
### [2026-08-12] project: 赚钱线启动 —— owner 要 Claude 帮找线上接单/赚钱机会，以赚钱为优先
owner 原话「帮我去找工作赚钱 以赚钱为优先 找线上接单平台等 任何喜好问题可以问我」。这是 §3「#1 = 交付一个结果」第一次直接以**钱**为 unit 落地（此前立项路径是多平台社媒）。四个待定项已发问：新加坡身份（SP 持有者受 MOM 限制不可自雇/freelance，决定路线合法边界）/ 市场（国际 USD vs 国内 RMB vs SG 本地受雇）/ 每周时间投入 / 快钱（AI 数据标注类）vs 高单价（Upwork AI-agent 细分）。方向锁定后扩写本条或立 sub-MD。Claude 边界：调研筛平台、写 profile/proposal/作品集文案、盯单可做；**注册账号、收付款、KYC 一律 owner 自己操作**。
**同日锁定**：owner 答 = **PR/公民**（MOM 限制不适用，freelance 全合法）、只打国际 USD、10–20h/周、两条腿。平台核实（2026-08-12 web 查证）：**DataAnnotation 出局**（只收美加英爱澳新六国）；主攻 **Mercor**（全球远程、日发 $1.5M、coding 评审均价 ~$31/h+）+ micro1，Outlier/Alignerr 打底；腿 B = Upwork AI-agent 细分（起步 $30/h，前 3–5 单攒评价）。落地包（平台表 / week-1 清单 / EN resume / Upwork profile / proposal 模板）→ [projects/freelance/FREELANCE.md](projects/freelance/FREELANCE.md)，后续项目记忆写那里。

### [2026-08-12] feedback: UI 实现优先用苹果官方预设/组件/材质/动画，不要手写等价物
Ripple FanLab 施工中 owner 中途纠偏：「你的 UI 和真正优秀的产品 UI 差距还是很大的，多使用苹果的预设以及动画设置，多用官方的内容，不要自己写。」与 [2026-07-01]「美学微调交人做」同族，但这条管的是**我动手写 UI 时的实现选材**：凡苹果有官方等价物的层，一律用官方的——动画用 `.snappy/.smooth/.bouncy` 预设不手调 spring 参数；触感用 `.sensoryFeedback` 不手管 UIKit generator；数字换值用 `.contentTransition(.numericText())`；抬升/玻璃用系统 Material 不手挑高亮 hex；字号用系统 text style（带 Dynamic Type）+ `minimumScaleFactor` 不写死 pt；图表用 Swift Charts。**边界**：官方没有的交互（如单指绕心拖拽的轮盘）几何自算没问题——错的是在官方有预设的层自造。呼应 [2026-08-09] 活动环那次的「只按我们的设计抄它的 UI 和动画」：owner 对「苹果的做工」是一贯要求，检查清单化：写完一屏 UI 自查一遍「哪几处我手写了苹果有现成的东西」。
**同日第二轮（owner 看到第一版实现后：「非常生硬」）—— 追加两条**：(1) **自己画的形状也要按平台做工画，圆角不是装饰**：环形扇区苹果确实无等价物、只能手绘，但四个尖角是「生硬」的最大来源——iOS 上没有任何控件带尖点，带尖点的形状被读成示意图而不是控件（倒角做法 = 沿两条边回切后用二次曲线经过原顶点闭合，圆角矩形的同一套构造搬到曲边上）。(2) **「像参考图」要连信息密度一起抄**：参考图只展开三片花瓣，我一次摊开五片跨 220°，两端标签几乎立起来还顶进页头——抄版式时把「它同时显示几个」也当成设计决定，不是可自由放大的参数。


### [2026-08-12] project: Ripple 新 UI 方向 = 扇形局部环 rotary picker（RingLab 已归档）
owner 2026-08-12 定：**RingLab 四台归档**（`ripple-ios/archive/RingLab/`，含 RingGeometry + 其 18 条测试，已移出所有 build target），理由是「新设计长的圆环太过于复杂」。新方向 = **只展示圆环的一部分**（参考图：宠物健康 app 的扇形选择器，owner 明确「用我们的色调，只是排版和 UI 可以学习」）：花瓣从中心圆扇出、**整个轮盘可拖动旋转**、停在底部的那片即选中。
落点 = 新 bench `RippleApp/FanLab/`（`RIPPLE_SCREEN=fanLab`，与 HealthProbe 同样「不是屏幕、整目录可删」）：`FanGeometry.swift`（SwiftUI-free 的选择数学：花瓣角度/旋转吸附/atan2 跨缝/橡皮筋/命中测试，18 条单测）+ `FanLabData.swift`（五路真信号假数据，含 SE 无血氧传感器那一路）+ `FanLabView.swift`。**轮盘是 picker 不是钟**——不编码时长，所以旧 24h 环撞的「一小时弧长 32pt < 44pt」那堵墙在这里不存在（花瓣弦长 ≈119pt）。已装 owner 真机（YoRHa, iOS 26.5.2）跑起来，拖动手感待 owner 判断。逐轮细节见 `ripple-ios` commit 6bd11a1 / 1596a70。
