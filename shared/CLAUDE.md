...
| ⬜ **未指派** | Inception · conversion-site · short-video · MoyuanIdea（04-03 后零更新，疑似休眠但从未正式宣告） · **StudyOS**（别人的项目；owner 只有一件事要做：审 [PR #2](https://github.com/yorhagengyue/study-os/pull/2)） |
...
| **StudyOS** | **别人的项目** · 基础版已交付、两台机器真机跑通 · **卡在 owner 审 PR #2**（2026-09-10 开至今，仓库自那天起无新提交） | 耿越给**时浩然**做的个人学习系统（**学习者 ≠ 制作人**；四门悉尼大学课 ACCT2011/2019 + IBUS2020/2105 **不得当成 owner 本人学籍**，对应 record 仓 DRAWER `c-9f1e3002`）。两部分：Agent 端直接在仓库里工作 + 本机网页端 `127.0.0.1:8765` 浏览/刷新 Canvas 资料（**不含 AI 对话**）。纯 Python 标准库零依赖 · Canvas 只读（不提交/不答题/不签到/不读成绩）· `student/` 同步永不改写 · **改动一律走 PR，只有耿越能合并**。实跑 246–247 文件 / 142 页面 / 338MB，未取得的 13 文件+6 处列表限制如实标注。**本机未 clone** | → [STUDY-OS.md](projects/study-os/STUDY-OS.md) |
