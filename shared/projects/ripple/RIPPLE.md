...
> **最后更新**：2026-08-12（RingLab 归档 → FanLab 扇形局部环 bench 上线并装真机，见决策历史 08-12 条；同日 onboard 核实：`gengyue/figma-baseline` 已 push 同步。此前 2026-08-09：补设计师交接线）
...
**⚠️ 以下 RingLab 全套已于 2026-08-12 归档到 `ripple-ios/archive/RingLab/`（owner：圆环太复杂，改做「扇形局部环」rotary picker，见决策历史 08-12 条）。硬约束数据仍有效，留作史料。**

**圆环第一件落地（2026-08-09，owner 指示「单开一个页面做圆环，别直接写在 app 里面；冷启动暂用现有 UI，后续再定」）**：`ripple-ios/RippleApp/RingLab/`（`RingGeometry.swift` 纯逻辑进 logic-test target + `RingLabData.swift` 假数据 + `RingLabView.swift` 台子），入口只有 dev-only 的 `RIPPLE_SCREEN=ringLab`，不在任何 tab、无人链接、整目录可删。15 条单测全绿、真机模拟器截图+点击命中实测通过（tap→Lecture 08:30–10:30 · 79pt along × 22pt across）。已随 7f8550c（08-11）提交并 push。
...
- **2026-08-25 — SUSS GIF 2026 长辈版 demo 冲刺（一夜线，真源 = `ripple-ios/docs/DEMO-SUSS-GIF.md`）**：明日（08-26）SUSS 老年创业创新节现场演示，owner 锁定「现有 app 加 Elder Mode + demo 种子账号 + 英文」，demo 内容由 Zicheng/Yufei 经 remote 进对话拍板。分支 `demo/suss-gif-2026`（不合 main）。Elder Mode v1 = root 级 Dynamic Type 提档（核心屏全语义字号所以一行生效）+ Me 页开关 + `-RIPPLE_ELDER` 启动覆盖 + LAST 7 DAYS 单列，拨动即全 app 实时换档。顺路修了两笔账：**demo 账号与 fixture 账号的种子都停在 8 月初**（近两周空数据；种子脚本 http 加传输层重试后重种全绿——Vercel 读会间歇性挂住是既有环境问题）；`seed_demo_elder.py`（Mr. Tan 长辈 persona）备好待批。截图评审走 public repo `yorhagengyue/suss-demo-shots`。会后：跑 `seed_demo_rich.py` 还原 Jordan、demo 分支处置待定。：owner 看参考图（宠物健康 app 的 fan selector）后定：完整圆环太复杂，**只展示圆环的一部分、可拖动旋转**；配色用 Ripple 自己的、只学排版。落地：`RippleApp/RingLab/` 四台 + RingGeometryTests **全套 git mv 到 `ripple-ios/archive/RingLab/`**（出编译范围，历史保留）；新建 `RippleApp/FanLab/`（FanGeometry 纯逻辑进 logic-test target 18/18 绿 + FanLabData 假数据 + FanLabView），入口 `RIPPLE_SCREEN=fanLab`（`RIPPLE_FAN_START=0..4` 供无头 QA 选起始瓣）。形态：hub 大圆停选中信号（名字+大数字+状态 pill），5 瓣 44° 扇形绕 hub，拖动旋转/点瓣切换，底部瓣即选中；瓣过水平线渐隐（否则端点态压 header，截图实证后修）；下方 LAST 7 DAYS（复用 WeekChart，身份色 per 信号）+ VS YOUR USUAL（Deviation 就地算 + usual 值全页唯一一次）；Blood O₂ 做成无传感器死格子案例。工况读数印在页脚：**petal chord ≈123pt ≥ 44pt**——旧 dial 的触控墙在 picker 形态下不存在。**owner 同日中途纠偏（已记 §8）：UI 实现多用苹果官方预设，不手写等价物**——已换 `.snappy`/`.sensoryFeedback(.selection)`/`.contentTransition(.numericText())`/系统材质/`.largeTitle`；轮盘几何本身无官方组件，仍自算。真机 YoRHa（iPhone 17 Pro Max）已装并拉起（`DEVICECTL_CHILD_RIPPLE_SCREEN=fanLab devicectl launch`）。⚠️ 同轮发现 pre-existing：LiveSmoke `testLiveDayBundleDecodes` 线上 seeded day 0 metrics（后端种子状态，与本改动无关，待查）。
...
0b. ~~push `gengyue/figma-baseline`~~ **已完成**（2026-08-12 核实与 origin 同步，含 71 张基线截图 + 08-11 的 7f8550c）。残留事实：分支领先 main 12 commit 未合并。
