...
## 📍 当前进度（synced 2026-07-25 · 真源 = repo `LOG.md`）

> ⚠️ 下面第二个「当前状态」段是 **2026-05-18 的历史**（route validation 阶段），不是现状；以本段为准。

**卡在哪**：停在 **2026-07-17 09:52**（最后一次 commit `3605774`），等 owner 给 S02 的生成/参考素材。此后至今无新动作。

**已定稿资产**：角色三视图（无表情修复版）、骨钉三视图（超写实灰暗锻铁）、A3 圣巢深渊俯瞰图、A4 苍白之王、A5 幼年兄长（A6 已并入 A5）、A7 深渊兄弟（单体×4 模式，材质口径=半液态将凝未凝黑釉）、A8 渊底面具之海（五图喂法：Mixed1-4 兄弟成品入景 + Mixed5 游戏截图为场景总权威）。

**镜头**：
- **S01「登临眺望」15s 已生成并经 owner 验收**（落幅=崖沿背影俯瞰灯海）。
- **2026-07-17 编号重整（owner 定）**：原 S01+S02 合并为 **S01（S01A 登临眺望 15s / S01B 纵身 5s）**；新 **S02 =「诞生之地」深渊记忆 4 镜**（出生→攀爬→见苍白之王与兄长→封门坠回），剧本 v1 已写，经 wiki×3 + Steam 讨论 + 解包资产交叉验证。
- S02 七条关键决策：记忆世界=苍白底黑剪影（与 S01 反色）／**角色无手指**（攀爬只用点·勾·挂·贴壁蠕行）／初生形态无披风无骨钉／四行碑文=后期叠字不进生成（反讽对位：字说无，画面证明有）／兄长回头与主角对视／封门瞬间主角**自己松开**／白王永不给正脸。

**方法论**（踩坑换来，后续镜头沿用，全文在 repo `LOG.md`）：提示词全中文·片场通告单级密度；参考图要显式教「锁死什么／忽略什么／只借什么」；**视角不对先换参考图**（参考图透视永远赢过文字）；迭代用「一切不变，只改 X」短提示词；世界观无日月星辰；**Q 版比例靠数学锁死**（像素实测官方原图 → 整数等分比 + 关键宽度分数 + 日常物件意象锚，A4 白王一把过）。

**资料层**：Obsidian 活 vault `09 - 视频/aitv-hollow-knight/` 两篇（《空洞骑士世界编年史-主角视角》《空洞骑士主角线-小骑士全传》，2026-07-16）—— 这是全 vault 最新的写入，别因"vault 已是存档层"而漏查。
