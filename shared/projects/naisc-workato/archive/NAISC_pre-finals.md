...
> **Demo 录制**：2026-04-20
> **决赛**：2026-05-22
> **最后更新**：2026-05-08 00:50 SGT（mentor 会议转录）
...
## [2026-05-08] Mentor 会议录音 + 转录

**情境**：5/22 决赛前的 mentor 答疑，2 段录音（TP7 32:04 + TP8 34:48 = 67 min）。
**转录文件**（保留原始 SRT，已折叠 Whisper 幻觉循环）：
- `01 - Projects/Workato NAISC/transcripts/Recording · 2026-05-08 · TP7.md`（994/1054 段保留，12 处幻觉循环 ⚠️ 标记）
- `01 - Projects/Workato NAISC/transcripts/Recording · 2026-05-08 · TP8.md`（1362/1477 段保留，21 处幻觉循环 ⚠️ 标记）

**Whisper 失误备注**：
- 两份均被强制识别为 zh（概率 1.000），实际为中英混合 → 部分英文段落被生硬转中文，理解时需对照 SRT 时间戳
- TP7 早期 `00:00–08:00` "画面" 高密度区段未被自动折叠（保守阈值），需手动甄别
- TP8 最大 hallucination 是 `15:20–16:08` 的 45 段 "我认识" loop（已折叠）

**初步内容线索**（未深读，仅快扫所得，**用户判断为准**）：
- TP7 末尾用户向某人解释 Workato 项目背景（"company name is Wakato, met CEO in Japan school trip"）→ 推断这段是赛前向同行/朋友介绍
- TP8 开头是 mentor 给 Ripple 的产品建议："build a UI for family chat / family circle", "make it a community thing", "from a product's perspective it's actually very strong", "doesn't really showcase much of the technology"
- TP8 中段讨论 B2B vs B2C 投资逻辑、mental health & wellness 受众
- TP8 末尾 mentor 提到时间表：Wed 9-4（5 点有会）、Thu 11-6

**待用户决定**：
1. mentor 建议是否要 act on（community/family circle 改造 vs 保持当前 individual wellness 定位）
2. 是否要二次清理转录（更狠的幻觉折叠规则 / 把文件提取出 mentor 名字 / 抽问答对照）
3. 是否把"用户问 / mentor 答"分开整理成 Q&A 格式
